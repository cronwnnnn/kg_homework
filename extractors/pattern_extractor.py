...
新策略（vs 第一版）：
    1. head 取触发词左侧最近且最长的词典实体（而非贪心整段）；
    2. tail 取触发词右侧最近且最长的词典实体（也支持列举 "X、Y、Z"）；
    3. 触发词与实体的允许距离窗口受 _HEAD_LOOKBACK / _TAIL_LOOKAHEAD 限制；
    4. 单个触发词命中只产出一条三元组（除了列举式的 has_part 可多产）。
"""

...
| 领域词典 | 1473 个实体，分 14 类（`ans.py` / `data/entities_by_type.json`） |
| 自动抽取实体 | **670** |
| 自动抽取三元组 | **1584** |
| 人工金标（第四章） | 745 条关系（`gold/gold_triples_augmented.csv`） |
| **严格 F1** | **0.6813**（P=0.7985, R=0.5940） |
| **Partial F1** | **0.6859** |
| **实体级 F1** | **0.8032**（P=0.9716, R=0.6846） |
| `instance_of` F1 | **0.927** |
...
主线包含 **5 个独立的传统抽取算法**，由 `extractors/pipeline.py` 统一调度。
...
| 要求 | 阈值 | 当前 | 状态 |
|---|---|---|---|
| 概念（实体） | ≥500 | **1473**（词典） / **670**（实际抽取） | ✓ ×3.0 |
| 关系（三元组） | ≥1000 | **1584** | ✓ ×1.6 |
| 人工金标关系 | ≥400 | **745** | ✓ ×1.9 |
| 自动抽取算法源代码 | 必须 | `extractors/` 5 个算法 + LLM 增强 + 流水线 | ✓ |
| 不允许只用 LLM | 必须 | 主体由传统抽取产 1300 条，LLM 仅基于已有候选发现补全 290 条；关闭 LLM 仍可独立运行 | ✓ |
